...
import CharacterUsers from "./CaracterUsers.js";
import CharacterMe from "./CaracterMe.js";
...
export default function MapArea({ mapRef, users, me, setMe, background, socket,audioEnabled }) {
  const STEP = 10;
  ...
  useEffect(() => {
    ...
    window.addEventListener("keyup", () => setMe((m) => ({ ...m, moving: false })));
    return () => {
      window.removeEventListener("keydown", handleKey);
      window.removeEventListener("keyup", () => setMe((m) => ({ ...m, moving: false })));
    };
  }, [handleKey, setMe]);

  return (
    <div ref={mapRef} onDoubleClick={onMapClick} className="map">
      {/* Renderiza outros usuários */}
      {Object.entries(users)
        ...
        .map(([id, u]) => (
          <CharacterUsers
            ...
            audio={u.audioEnabled} // ✅ cada user tem seu próprio estado
          />
        ))}
      ...
      <CharacterMe
        ...
        audio={me.audioEnabled} // ✅ usa só o seu
      />
      ...
      <div className="list">
        {Object.entries(users).map(([id, u]) => (
          <li
            ...
            style={{
              background:
                u.id === me.id ? "rgba(106, 255, 0, 0.46)" : "rgba(214, 212, 212, 0.63)",
            }}
          >
            {u.name} {u.audioEnabled === false && "🔇"}
          </li>
        ))}
      </div>
    </div>
